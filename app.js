...
const translations = {
	ru: {
		...
		// Alerts
		addTracksOffline:
			'Используйте перетаскивание файлов (drag & drop) для добавления треков офлайн',
		...
		if (navigator.storage && navigator.storage.persist) {
			try {
				...
				if (!isPersisted) {
					...
					if (granted) {
						console.log(
							'✅ Persistent storage GRANTED - Maximum quota unlocked!'
						)
					...
					console.log(
						'✅ Persistent storage already active - Maximum quota enabled'
					)
				}
			} catch (error) {
		...
		if (navigator.storage && navigator.storage.estimate) {
			try {
				...
				const estimatedSongs = Math.floor(
					availableSpace / (avgSongSizeMB * 1024 * 1024)
				)
				console.log(
					`🎵 Estimated capacity: ~${estimatedSongs.toLocaleString()} songs (${avgSongSizeMB}MB each)`
				)
				console.log('═══════════════════════════════════════')
			} catch (error) {
...
class MusicPlayer {
	...
	initEventListeners() {
		...
		this.addTracksBtn.addEventListener('click', () =>
			this.handleAddTracksClick()
		)
		...
		const preventBackgroundPause = (element, name) => {
			element.addEventListener('pause', () => {
				...
				if (
					...
					document.hidden &&
					!this.isResuming &&
					!this.isChangingTrack
				) {
					console.log(
						`${name} paused while hidden - resuming for background playback`
					)
	...
	updateOnlineStatus(isOnline) {
		...
		this.statusText.textContent = isOnline
			? t('statusOnline')
			: t('statusOffline')
		...
		try {
			for (const file of files) {
				if (
					audioOnly &&
					(file.type.startsWith('video/') ||
						file.name.match(/\.(mp4|m4v|webm)$/i))
				) {
		...
		this.songList.innerHTML = this.playlist
            ...
            <div class="song-item ${
                ...
                <div class="song-item-info">
                    ...
                    <div class="song-item-duration">${this.formatFileSize(
											song.size
										)}</div>
                </div>
	...
	async clearAllSongs() {
		...
		try {
			...
			this.currentSongTitle.textContent =
				currentLang === 'ru' ? 'Трек не играет' : 'No track playing'
			this.currentSongArtist.textContent =
				currentLang === 'ru'
					? 'Выберите трек для начала'
					: 'Select a track to start'
			this.vinylDisc.classList.remove('spinning')

			this.statusText.textContent =
				currentLang === 'ru' ? 'Все треки удалены' : 'All tracks deleted'
			setTimeout(() => {
				this.updateOnlineStatus(navigator.onLine)
			}, 2000)
		} catch (error) {
	...
	async playSongAtIndex(index) {
		...
		try {
			...
			const isVideo =
				songData.type.startsWith('video/') ||
	...
	async showStorageInfo() {
		...
		if (navigator.storage && navigator.storage.estimate) {
			try {
				...
				console.log(
					`📊 Storage: ${usedMB} MB / ${quotaMB} MB (${percentUsed}%)`
				)
